...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    // Игнорируем слэш-команды — их обрабатывает index.js
    ...
    try {
      ...
      if (interaction.isButton()) {
        ...
        if (id === 'emias:admit') return handleAdmitButton(interaction);
        if (id === 'emias:prescription') return handlePrescriptionButton(interaction);

        // Штаб
        if (id === 'staff:add') return handleStaffAddButton(interaction);
        if (id === 'staff:list') return handleStaffList(interaction);
        if (id === 'staff:stats') return handleStats(interaction);
      ...
      if (interaction.isStringSelectMenu()) {
        if (id === 'emias:status:select') return handleStatusSelect(interaction);
        if (id === 'staff:wipe:select') return handleWipeSelect(interaction);
      }

      // ─── Modals ─────────────────────────────────────────────────────
      if (interaction.isModalSubmit()) {
        if (id === 'emias:book:modal') return handleBookModal(interaction);
        if (id === 'emias:link:modal') return handleLinkModal(interaction);
        if (id === 'emias:card:modal') return handleCardModal(interaction);
        if (id === 'emias:admit:modal') return handleAdmitModal(interaction);
        if (id === 'emias:prescription:modal') return handlePrescriptionModal(interaction);
        if (id === 'staff:add:modal') return handleStaffAddModal(interaction);
...
async function handleBookButton(interaction) {
...
async function handleBookModal(interaction) {
  ...
  // Пациент
  const citizens = emias.getCitizensByDiscordId(interaction.user.id);
  ...
  const patient = emias.getPatientById(pid);
  if (!patient || patient.discord_id !== interaction.user.id) throw new Error('Персонаж не найден или не ваш.');

  const res = emias.bookAppointment({ patientId: pid, doctorId, date: dateRaw, time: timeRaw, viaDiscordId: interaction.user.id });
...
async function handleTickets(interaction) {
  ...
  for (const ch of citizens) {
    const card = emias.getPatientCard(ch.id);
...
async function handleLinkModal(interaction) {
  ...
  try {
    const p = emias.linkPatientByCode(code, interaction.user.id);
...
async function handleQueue(interaction) {
  await interaction.deferReply({ flags: FLAGS, ephemeral: true });
  const staff = emias.getUserByDiscordId(interaction.user.id);
...
async function handleCardModal(interaction) {
  ...
  const card = emias.getPatientCard(pid);
...
async function handleStatusSelect(interaction) {
  ...
  emias.setDoctorStatus(interaction.user.id, status);
...
async function handleAdmitButton(interaction) {
  const staff = emias.getUserByDiscordId(interaction.user.id);
...
async function handleAdmitModal(interaction) {
  await interaction.deferReply({ flags: FLAGS, ephemeral: true });
  const staff = emias.getUserByDiscordId(interaction.user.id);
  ...
  const id = emias.addEmrRecord({ patientId: pid, doctorId: staff.id, recordType: 'visit', complaints, diagnosisCode: code, diagnosisText: text, notes, sickLeaveDays: sick });
...
async function handlePrescriptionButton(interaction) {
  const staff = emias.getUserByDiscordId(interaction.user.id);
  if (!staff || ![ROLES.PHYSICIAN, ROLES.HEAD_PHYSICIAN].includes(staff.role)) {
    await interaction.reply({ components: [errorContainer('Только для врачей.')], flags: FLAGS, ephemeral: true });
    return;
...
async function handlePrescriptionModal(interaction) {
  await interaction.deferReply({ flags: FLAGS, ephemeral: true });
  const staff = emias.getUserByDiscordId(interaction.user.id);
  if (!staff || ![ROLES.PHYSICIAN, ROLES.HEAD_PHYSICIAN].includes(staff.role)) throw new Error('Только для врачей.');
  const pid = Number(interaction.fields.getTextInputValue('pid').trim());
  ...
  const res = emias.addPrescription({ patientId: pid, doctorId: staff.id, medication: med, dosage, durationDays: days });
...
async function handleStaffAddButton(interaction) {
  const actor = emias.getUserByDiscordId(interaction.user.id);
  const all = emias.getAllStaff();
...
async function handleStaffAddModal(interaction) {
  await interaction.deferReply({ flags: FLAGS, ephemeral: true });
  ...
  if (emias.getUserByDiscordId(discordId)) throw new Error('Пользователь уже в штате.');
  const id = emias.createStaff({ discordId, discordUsername: null, fullName: fio, specialty, role });
...
async function handleWipeSelect(interaction) {
  await interaction.deferUpdate();
...
async function handleBlockModal(interaction) {
  ...
  const p = emias.getPatientById(pid);
  if (!p) throw new Error('Пациент не найден.');
  emias.setPatientBlocked(pid, true, actor.id);
...
async function handleUnblockModal(interaction) {
  ...
  const p = emias.getPatientById(pid);
  if (!p) throw new Error('Пациент не найден.');
  emias.setPatientBlocked(pid, false, actor.id);
